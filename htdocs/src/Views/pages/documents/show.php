<?php
/**
 * DocuFlow - Vue document avec traductions FR/EN intégrées
 * La langue est détectée via $_SESSION['language']
 */
...
<div class="document-viewer-page">
    <div class="document-header">
        ...
        <div class="document-header-actions">
            ...
            <a href="/uploads/<?= $document['filename'] ?? '' ?>" target="_blank" class="btn btn-ghost btn-sm" title="<?= _t('download') ?>">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            </a>
            <a href="/documents/<?= $document['id'] ?>/edit" class="btn btn-ghost btn-sm" title="<?= _t('edit') ?>">
    ...
    <div class="document-viewer-container">
        <div class="pdf-viewer-panel">
            <div class="pdf-toolbar">
                <div class="pdf-nav">
                    <button onclick="previousPage()" id="prevBtn" disabled><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg></button>
                    <span class="page-info">Page <input type="number" id="pageInput" value="1" min="1"> / <span id="pageCount">1</span></span>
        ...
        <div class="document-sidebar">
            ...
            <div class="sidebar-content">
                <div class="tab-content active" id="tab-zones">
                    <div class="tab-header"><h3><?= _t('selected_zones') ?></h3><span class="badge"><?= count($zones) ?></span></div>
                ...
                <div class="tab-content" id="tab-annotations">
                    ...
                    <div class="annotations-list"><?php if (empty($annotations)): ?><div class="empty-state small"><p><?= _t('no_annotations') ?></p></div>
                    <?php else: foreach ($annotations as $a): ?><div class="annotation-item <?= !empty($a['is_resolved']) ? 'resolved' : '' ?>" data-annotation-id="<?= $a['id'] ?>"><div class="annotation-header"><span><?= htmlspecialchars($a['first_name'] ?? '') ?></span><span class="badge"><?= $a['annotation_type'] ?? '' ?></span></div><div class="annotation-content"><?= nl2br(htmlspecialchars($a['content'] ?? '')) ?></div><div class="annotation-footer"><span><?= isset($a['created_at']) ? date('d/m/Y', strtotime($a['created_at'])) : '' ?></span><?php if (empty($a['is_resolved'])): ?><button onclick="resolveAnnotation(<?= $a['id'] ?>)" class="btn btn-xs"><?= _t('resolve') ?></button><?php endif; ?></div></div><?php endforeach; endif; ?></div>
                </div>
                
                <div class="tab-content" id="tab-info">
                    <div class="info-section"><h4><?= _t('document_details') ?></h4><dl class="info-list"><dt><?= _t('filename') ?></dt><dd><?= htmlspecialchars($document['original_name'] ?? $document['filename'] ?? '') ?></dd><dt><?= _t('file_size') ?></dt><dd><?= isset($document['file_size']) ? number_format($document['file_size']/1024, 0) . ' Ko' : '-' ?></dd><dt><?= _t('type') ?></dt><dd><?= \App\Models\Document::TYPES[$document['document_type']] ?? _t('document') ?></dd><dt><?= _t('created_on') ?></dt><dd><?= isset($document['created_at']) ? date('d/m/Y', strtotime($document['created_at'])) : '-' ?></dd></dl></div>
                </div>
            </div>
        </div>
    </div>
...
<div class="form-group"><label><?= _t('extracted_text') ?></label><textarea id="zoneText" rows="2" readonly></textarea></div>
</div>
...
const LANG = { zone: '<?= _t('zone') ?>', delete_zone_confirm: '<?= _t('delete_zone_confirm') ?>', ocr_confirm: '<?= _t('ocr_confirm') ?>', ocr_complete: '<?= _t('ocr_complete') ?>', error_zone_not_found: '<?= _t('error_zone_not_found') ?>', select_document: '<?= _t('select_document') ?>', content_required: '<?= _t('content_required') ?>', select_user: '<?= _t('select_user') ?>', notification_sent: '<?= _t('notification_sent') ?>', entire_document: '<?= _t('entire_document') ?>' };
...
async function deleteZone(id) { if (!confirm(LANG.delete_zone_confirm)) return; await fetch(`/api/zones/${id}/delete`, { method: 'POST' }); zones = zones.filter(z => String(z.id) !== String(id)); renderZones(); document.querySelector(`[data-zone-id="${id}"]`)?.remove(); }

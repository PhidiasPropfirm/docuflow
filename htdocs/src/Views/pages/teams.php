<?php
/**
 * DocuFlow - Page de gestion des équipes
 * Fichier: htdocs/src/Views/pages/teams.php
 * 
 * CORRECTIONS:
 * - Comptage correct des membres (utilise count($team['members']) si member_count = 0)
...
/**
 * Helper pour obtenir le nom de l'équipe traduit
 */
if (!function_exists('getTeamName')) {
    function getTeamName(array $team): string {
        $lang = function_exists('currentLang') ? currentLang() : ($_SESSION['lang'] ?? 'fr');
...
/**
 * Helper pour obtenir la description de l'équipe traduite
 */
if (!function_exists('getTeamDescription')) {
    function getTeamDescription(array $team): string {
        $lang = function_exists('currentLang') ? currentLang() : ($_SESSION['lang'] ?? 'fr');
...
/**
 * Helper pour calculer le nombre de membres
 */
function getTeamMemberCount(array $team): int {
    // Priorité à count(members) si disponible
    if (!empty($team['members']) && is_array($team['members'])) {
        return count($team['members']);
    }
    // Sinon utiliser member_count
    return (int)($team['member_count'] ?? 0);
}
?>
...
<div class="teams-grid">
    ...
    <?php $memberCount = getTeamMemberCount($team); ?>
    <div class="team-card">
        <div class="team-card-header">
            ...
            <div class="team-card-actions">
                ...
                <!-- Bouton Supprimer -->
                <button onclick="confirmDeleteTeam(<?= $team['id'] ?>, '<?= htmlspecialchars(addslashes(getTeamName($team)), ENT_QUOTES) ?>')" class="btn btn-ghost btn-danger-ghost btn-xs" title="<?= __('delete') ?>">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="modal-body">
                ...
                <div class="form-row-2">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="teamNameEn" name="name_en">
                    </div>
                </div>
                ...
                <div class="form-row-2">
                    <div class="form-group">
                        ...
                        <textarea id="teamDescription" name="description" rows="3"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="teamDescriptionEn"><?= __('team_description') ?> (EN)</label>
                        <textarea id="teamDescriptionEn" name="description_en" rows="3"></textarea>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="color-picker-wrapper">
                        <input type="color" id="teamColor" name="color" value="#3B82F6" onchange="updateColorPreview(this.value)">
                        <div class="color-preview" id="colorPreview" style="background-color: #3B82F6;"></div>
                    </div>
                </div>
            </div>
...
<div class="modal" id="deleteModal">
    ...
    <div class="modal-content" style="max-width: 400px;">
        ...
        <form action="" method="POST" id="deleteForm">
            <?= csrf_field() ?>
            <div class="modal-body">
                <p><?= __('delete_team_confirm') ?></p>
                <p style="font-weight: 600; margin-top: 8px;" id="deleteTeamName"></p>
            </div>
            <div class="modal-footer">
                <button type="button" onclick="closeDeleteModal()" class="btn btn-secondary"><?= __('cancel') ?></button>
...
function openTeamModal() {
    ...
    document.getElementById('teamModalTitle').textContent = '<?= __('new_team') ?>';
    document.getElementById('teamForm').reset();
    document.getElementById('teamForm').action = '/teams'; // Création
...
function editTeam(team) {
    document.getElementById('teamModal').classList.add('active');
    document.getElementById('teamModalTitle').textContent = '<?= __('edit_team') ?>';
    document.getElementById('teamForm').action = '/teams/' + team.id; // Mise à jour
    ...
    updateColorPreview(team.color || '#3B82F6');
}
...
function confirmDeleteTeam(teamId, teamName) {
    ...
    document.getElementById('deleteForm').action = '/teams/' + teamId + '/delete';
}

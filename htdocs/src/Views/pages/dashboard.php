 ...
 * 
 * MISE À JOUR: Traduction dynamique des descriptions d'activité intégrée
 */

/**
 * Traduit une description d'activité stockée en français vers la langue courante
 */
if (!function_exists('translateActivityDescription')) {
    function translateActivityDescription(string $description): string {
        // Si on est en français, retourner tel quel
        if (currentLang() === 'fr') {
            return $description;
        }
        ...
        $translations = [
            // Upload
            '/^Upload du document:\s*(.+)$/i' => 'Document upload: $1',
            '/^Upload de document:\s*(.+)$/i' => 'Document upload: $1',
            
            // Suppression
            '/^Suppression du document:\s*(.+)$/i' => 'Document deleted: $1',
            '/^Suppression de document:\s*(.+)$/i' => 'Document deleted: $1',
            
            // Modification
            '/^Modification du document$/i' => 'Document updated',
            '/^Modification du document:\s*(.+)$/i' => 'Document updated: $1',
            
            // Zones
            '/^Création de zone sur le document #(\d+)$/i' => 'Zone creation on document #$1',
            '/^Modification de zone$/i' => 'Zone updated',
            '/^Suppression de zone$/i' => 'Zone deleted',
            
            // Liaisons
            '/^Création de liaison$/i' => 'Link created',
            '/^Suppression de liaison$/i' => 'Link deleted',
            
            // Annotations
            '/^Ajout d\'annotation$/i' => 'Annotation added',
            '/^Résolution d\'annotation$/i' => 'Annotation resolved',
            '/^Suppression d\'annotation$/i' => 'Annotation deleted',
            
            // Authentification
            '/^Connexion réussie$/i' => 'Login successful',
            '/^Déconnexion$/i' => 'Logout',
            
            // Utilisateurs
            '/^Création d\'utilisateur:\s*(.+)$/i' => 'User created: $1',
            '/^Modification d\'utilisateur$/i' => 'User updated',
            '/^Désactivation d\'utilisateur$/i' => 'User deactivated',
            
            // Équipes
            '/^Création d\'équipe:\s*(.+)$/i' => 'Team created: $1',
            '/^Modification d\'équipe$/i' => 'Team updated',
            '/^Suppression d\'équipe$/i' => 'Team deleted',
            
            // Mentions
            '/^Mention envoyée à l\'utilisateur #(\d+)$/i' => 'Mention sent to user #$1',
            '/^Mention envoyée$/i' => 'Mention sent',
        ];
                    ...
                    <a href="/documents/<?= $doc['id'] ?>" class="document-item">
                        ...
                        <div class="document-info">
                            ...
                            <span class="document-meta">
                                <span class="badge badge-<?= $doc['document_type'] ?>"><?= __('type_' . $doc['document_type']) !== 'type_' . $doc['document_type'] ? __('type_' . $doc['document_type']) : (\App\Models\Document::TYPES[$doc['document_type']] ?? __('other')) ?></span>
                    ...
                    <div class="activity-item">
                        ...
                        <div class="activity-content">
                            <span class="activity-text">
                                ...
                                <?= sanitize(translateActivityDescription($activity['description'] ?? '')) ?>
                            </span>
